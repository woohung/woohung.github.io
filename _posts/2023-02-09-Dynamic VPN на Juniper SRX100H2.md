...
Теперь используем команду `commit` и готово.  
...
Наша версия очень старая, поэтому необходимо сначала обновиться до промежуточноый, а затем до последней. Качаем нужный софт, далее заливаем в железку. Я делал с помощью WinSCP по FTP, предварительно включив его командой: (не забудьте выключить после)  
```
...
Проверяем версию BIOS (с новой прошивкой будет и новая версия BISO)  
```
